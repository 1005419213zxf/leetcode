...
public class DisplayAdwordsMetricProvider extends AdwordsMetricProviderBase implements AdwordsMetricProviderInterface {

	// constructor
	public DisplayAdwordsMetricProvider(Date endDate, int interval) throws SQLException, IOException{
...
//		init(endDate, interval, AdwordsDevice.mobile);		
	...
	private void init(Date endDate, int interval, AdwordsDevice aDevice) throws IOException{
		String InputDir = DirDef.getFormattedReportDir(endDate, AdwordsReportType.campaign_device_performance, interval
				, aDevice);
		for(String inputFilePath : FileHelper.getFilePathsInOneDir(InputDir)){		
			...
			while((line = br.readLine()) != null){
			    ...
			    if(!thisChannel.equals(AdwordsChannel.display)){
			    	continue;
			    }
				...
				AdwordsDevice thisDevice = attribute.getDevice();
				mergeDataToMap(report, attribute, thisDevice, thisChannel,siteType,languageType, cid, accountId, campaignId);
			}
			br.close();
		}
	}
	...
	public static void main(String[] args) throws IOException, ParseException, AdwordsValidationException, AdwordsApiException, AdwordsRemoteException {
		...
		System.out.println();
		try {
			DisplayAdwordsMetricProvider provider = new DisplayAdwordsMetricProvider(endDate, interval);
