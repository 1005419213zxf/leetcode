...
public class DelayRateProvider implements DelayRateProviderInterface {
	...
	// constructor
	public DelayRateProvider(SiteType siteType) throws NumberFormatException, IOException, SQLException  {
...
//		CPTree miniCpTree = new CPTree(dbHelper, onlyCategory, onlyValidCategory, considerSoftLink, considerSoftCopy, productStatus);
		DelayRateProvider litbProvider = new DelayRateProvider(SiteType.litb);
		DelayRateProvider miniProvider = new DelayRateProvider(SiteType.mini);
		...
		while(in.hasNext()) {
			try {
				...
				if(line.substring(0, 3).equals("all")) {
					...
					for(DelayRateProvider provider : new DelayRateProvider[] {litbProvider, miniProvider}) {
				...
				for(int getOffsetDays=1; getOffsetDays<=30; getOffsetDays++) {
					DelayRateProvider provider = (siteType == SiteType.litb) ? litbProvider : miniProvider;
					info = provider.getDelayRate(channel, siteType, languageType, categoryId, daysInterval, getOffsetDays, 30);
					if(info != null) {
